...
def failure_indicator(y_data, threshold=0.5):
...
def calculate_mcmc_sampling(X_test, y_test, conf_level=0.80):
    ...
    for i in range(1000, len(X_test)+1, 1000):
        # 读取 0 到 i 的数据
        ...
        # error = calculate_relative_error(gt_faulure=0.000070, failure_estimate=failure_estimate, n_samples=i)
    ...
    axes[1].axhline(y=0.10, color='red', linestyle='--', label='Target Accuracy')
    axes[1].set_xlabel('Number of samples')
    axes[1].set_ylabel('CMC relative error')
    axes[1].set_ylim(0.02, min(max(mcmc_errors[2:], default=10.0), 1.0))
    axes[1].grid(True)
    axes[1].legend()
    ...
    # 将结果写入xlsx文件
    excel_path = os.path.join('../../results/s6exp/SamplingResults.xlsx')
...
if __name__ == "__main__":
    ...
    # 加载测试结果
    file = '../../results/MCMC/scenario06/sampled_parameters.pkl'
    X_data = load_data(file)
    file = '../../results/s6exp/MCMC_simulation_scores.pkl'
    y_data = load_data(file)

    calculate_mcmc_sampling(X_data,y_data, conf_level=0.95)
