...
def log_q_mix_density_defensive(samples, q_ce, alpha, Z_GMM):
    """
    全校准版的防御性混合密度计算 - 全局均匀分布防御
    log_q_mix = log( (1-alpha)*(q_ce/Z) + alpha*Uniform([-1,1]^D) )
    """
    # 1. 计算 GMM 校准对数概率
    log_q_ce_norm = q_ce.score_samples(samples) - np.log(Z_GMM)

    # 2. 计算全局均匀分布的对数概率
    # 在 [-1, 1]^D 范围内，体积 V = 2^D
    dim = samples.shape[1]
    vol = 2.0 ** dim
...
def update_surrogate_model(samples, true_values, gp_model, gp_likelihood):
    ...
    # 保存
    save_path = '/home/hp/SENSE/rlsan/results/s1exp/last_updated_surrogate_model.pkl'
...
def adaptive_diversity_selection(X_candidates, u_scores, u_threshold=2.0, n_max_sim=200):
    ...
    return uncertain_indices[selected_in_uncertain]


def cross_entropy_refine_robust(q_init, gp_model, gp_likelihood, runner, N=10000, Ne_init=1000, eta = 0.5, T=10, lambda_mix=0.5):
    """
    ...
    for t in range(T):
        ...
        # 【修改点 3】调整 lambda_mix 动态逻辑：更慢的衰减，保持警觉
        if ESS_ratio < 0.3:
            # 权重太集中，增加由于防御分布的权重
...
if __name__ == "__main__":
    ...
    # 1. 加载 Surrogate GP
    gp_path = '../../results/s1exp/surrogate_model.pkl'
    ...
    log_q_mix_values = log_q_mix_density_defensive(X_final, final_q_ce, alpha=final_lambda_mix, Z_GMM=final_Zt)
    save_package = {
        'gmm': final_q_ce,
