...
class ADScenarioEnv(BaseOptEnv):
    ...
    def __init__(self, 
        ...
        self.surrogate_final_count = 0          # 新增：最终使用代理预测的次数
        self.real_simulation_final_count = 0    # 新增：最终使用真实仿真的次数
    ...
    def evaluate(self, positions: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # 初始假设所有位置都使用代理预测
        self.surrogate_final_count += len(positions)
        real_sim_count_before = self.real_simulation_count
        ...
        if self.runner is not None:
            ...
            if np.any(high_uncertainty_mask):
                ...
                # 统计有多少位置被真实仿真替代
                num_replaced = self.real_simulation_count - real_sim_count_before
                self.surrogate_final_count -= num_replaced
                self.real_simulation_final_count += num_replaced
    ...
    def _refine_with_simulation(self, positions: np.ndarray,
        ...
        注意：real_simulation_count 会在_call_real_simulation中增加
        """
    ...
    def _call_real_simulation(self, positions: np.ndarray) -> np.ndarray:
        ...
        注意：不增加evaluation_count，因为已在evaluate()中统计过
        """
        results = []
    ...
    def get_info(self) -> dict:
        ...
        info.update({
            ...
            'surrogate_final_count': self.surrogate_final_count,
            'real_simulation_final_count': self.real_simulation_final_count,
    ...
    def reset_counters(self):
        ...
        self.real_simulation_final_count = 0
