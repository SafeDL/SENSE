...
def train_one_episode(pso: StateAwareNichePSO,
    ...
    # Reset temporal noise for deterministic exploration mapping each episode
    agent.reset_noise()
...
def plot_training_curves(rewards: list, actor_losses: list, critic_losses: list,
    ...
    axes[0, 1].set_title('Actor Loss (-Q)')
    axes[0, 1].grid(True, linestyle='--', alpha=0.5)
    
    # 3. 占位 (DDPG 没有 Alpha)
    axes[0, 2].text(0.5, 0.5, 'DDPG (No Entropy)', ha='center', va='center')
    axes[0, 2].axis('off')
