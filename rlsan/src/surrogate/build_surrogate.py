...
def build_surrogate_iteratively(X_data, y_data):
    ...
    for iteration in pbar:
        ...
        if iteration % 5 == 0:
            ...
            # 早停
            # if len(f1_scores) > 20 and np.var(np.array(f1_scores[-5:])) < 1e-4 and f1_scores[-1] > 0.92:
            #     print("Converged.")
            #     break
        else:
            if len(f1_scores) > 0: f1_scores.append(f1_scores[-1])

        # C. 采样 (全局)
        new_indices = acquisition_function(model, likelihood, X_data, active_mask,iteration, num_sim, top_n=8)
    ...
    plt.plot(range(0, len(f1_scores) * 5, 5)[:len(f1_scores)], f1_scores)
    plt.title('F1 Score (Single Global Exact GP)')
    plt.show()

    # 保存训练好的代理模型
    save_model(model, likelihood, '../../results/s8exp/surrogate_model_1000.pkl')
...
if __name__ == "__main__":
    ...
    with open('./train_data/scenario08_grid_x.pkl', 'rb') as f:
        grid_x = pickle.load(f)
        grid_x = grid_x[:80000,:]
    with open('./train_data/scenario08_grid_y.pkl', 'rb') as f:
        grid_y = np.array(pickle.load(f)).reshape(-1, 1)
    print(f"Data Loaded: X={grid_x.shape}, Y={grid_y.shape}")
    start_time = time.time()
    build_surrogate_iteratively(X_data=grid_x, y_data=grid_y)
    end_time = time.time()
