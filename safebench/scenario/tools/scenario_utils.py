...
def scenario_parse(config, logger):
    ...
    # 文件指向 config/scenario_type/xx.json文件,近似可以理解为融合了功能场景和行使路线的配置文件
    ...
    with open(list_of_scenario_config, 'r') as f:
        ...
        # 如果在yaml文件中,比如LC.yaml中定义了scenario_id,则在这里筛选出来
    ...
    # 按照地图将场景配置进行分类
    config_by_map = {}
    for item in data_full:  # data_full存储着所有的route_id,scenario_id等信息
...
def scenario_parse_simple(config, logger):
    """
        适配强化学习驱动的小生境搜索的简化版scenario_parse
    """
    ROOT_DIR = config['ROOT_DIR']
    logger.log(">> Parsing scenario route and data")
    # 文件指向 config/scenario_type/xx.json文件,近似可以理解为融合了功能场景和行使路线的配置文件
