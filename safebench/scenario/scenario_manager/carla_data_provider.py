"""
1、提供了从仿真器中获取和管理各种数据的方法,包括actor的速度、位置、变换、交通信号灯的位置和状态等
...
from six import iteritems  # 可以在不考虑Python版本的情况下迭代字典的键值对
...
class CarlaDataProvider(object): 
    ...
    def update_osc_global_params(parameters):
        """
            ...
            将一个字典的键值对更新到另一个字典中。如果键已经存在，则更新其值；如果键不存在，则添加新的键值对
        """
    ...
    def get_osc_global_param_value(ref):
        ...
        """
        # 将传入的参数ref中的"$"替换为空字符串，然后在全局的OSC参数中查找对应的值
    ...
    def get_velocity(actor):
        ...
        for key in CarlaDataProvider._actor_velocity_map:
            if key.id == actor.id:
                return CarlaDataProvider._actor_velocity_map[key]  # 单位m/s
    ...
    def get_next_traffic_light(actor, use_cached_location=True, use_transform=False):
        ...
        for traffic_light in CarlaDataProvider._traffic_light_map:
            if hasattr(traffic_light, 'trigger_volume'):
                tl_t = CarlaDataProvider._traffic_light_map[traffic_light]  # 这个代表traffic_light的transform
