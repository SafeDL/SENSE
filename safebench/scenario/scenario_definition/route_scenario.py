...
class RouteScenario:
    ...
    def __init__(self, world, config, ego_id, logger, max_running_step):
        ...
        self.list_scenarios = self._build_scenario_instances(scenario_definitions)  # 创建要执行的具体场景类的实例
    ...
    def _update_route_and_ego(self, timeout=None):
        ...
        if self.config.scenario_file is not None:
            world_annotations = RouteParser.parse_annotations_file(self.config.scenario_file)  # 这里的scenario_file指的是scenario_01.json这样的文件
        else:
        ...
        ego_vehicle = None
        route = None
        scenario_id = self.config.scenario_id  # 这里的scenario_id是一个整数,表示当前功能场景的ID
        if scenario_id == 0:
            vehicle_spawn_points = get_valid_spawn_points(self.world)
            # 从车辆生成点中随即选择一个作为自动驾驶车辆的生成点,一旦成功生成则退出循环,但是要注意的是,虽然写成random,但仍然是按照顺序选择的
            for random_transform in vehicle_spawn_points:
                ...
                if ego_vehicle is not None:
                    break
        else:  # 如果定义了固有场景,则根据XML定义的轨迹来生成主车
            route = interpolate_trajectory(self.world, self.config.trajectory)
            ego_vehicle = self._spawn_ego_vehicle(route[0][0], self.config.auto_ego)

        # 扫描出沿着给定测试路线的可能发生的功能场景,possible_scenarios是一个字典,键值为数字,值为场景定义的列表,包含‘match_position’,'name', 'trigger_position'等字段
        possible_scenarios, _ = RouteParser.scan_route_for_scenarios(
            self.config.town,
            route,  # route是一个列表,列表中的每个元素都是一个长度为2的tuple,包含有Transform和RoadOption
            world_annotations,  # world_annotations是一个字典,键为地图名称,值为路线上的功能场景名称和触发位置
            scenario_id=scenario_id  # scenario_id是一个整数,表示当前功能场景的ID
        )
        ...
        # 从多个场景定义中选择一个最接近给定路径起点的场景定义,以避免在同一个测试路径上找到了多个匹配的trigger point
        ...
        scenarios_definitions = [scenarios_definitions[scenarios_definition_id]]  # 将列表中索引为 scenarios_definition_id 的元素提取出来
    ...
    def get_running_status(self, running_record, cur_distance):
        running_status = {
            'ego_velocity': CarlaDataProvider.get_velocity(self.ego_vehicle),
            'ego_acceleration_x': self.ego_vehicle.get_acceleration().x,  #  m/s2
            'ego_acceleration_y': self.ego_vehicle.get_acceleration().y,  #  m/s2
            'ego_acceleration_z': self.ego_vehicle.get_acceleration().z,  #  m/s2
    ...
    def _create_criteria(self):
        ...
        #  Caution! 在这里指定要监听的碰撞对象,以排除agent的随机性错误
    ...
    def update_info(self):
        """
        这段代码定义了一个名为 update_info 的方法，用于获取自车（ego vehicle）和其他参与者（actors）的状态信息。具体操作如下：
            调用 _get_actor_state 方法获取自车的状态信息，并将其存储在 ego_state 变量中。
            创建一个列表 actor_info，并将 ego_state 添加到该列表中。
            遍历所有场景实例 self.list_scenarios，对于每个场景实例，进一步遍历其所有其他参与者 s_i.other_actors。
            对于每个其他参与者，调用 _get_actor_state 方法获取其状态信息，并将其添加到 actor_info 列表中。
            将 actor_info 列表转换为 NumPy 数组。
            返回一个包含 actor_info 的字典。
        """
