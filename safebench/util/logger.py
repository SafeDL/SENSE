...
class Logger:
    ...
    def add_eval_results_simple(self, scores=[], records=None):
        # 有几个测试场景,scores列表就有几个独立得分
        if len(scores) > 0:
            # 初始化计数器
            if not hasattr(self, 'eval_counter'):
                self.eval_counter = 0  # 初始化计数器
            # 将 scores 添加到 eval_results 中，使用计数器作为 key
            for score in scores:
                self.eval_results[self.eval_counter] = score
                self.eval_counter += 1
    ...
    def print_eval_results_simple(self, data_ids):
        for idx in range(len(data_ids) - 1, -1, -1):
            self.log(f"\n Evaluation {data_ids[idx]} results:")
            results_to_display = self.eval_results[self.eval_counter-idx-1]
            for key, value in results_to_display.items():
                self.log(f"\t {key: <25}{value}")
