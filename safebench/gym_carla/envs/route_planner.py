...
class RoutePlanner():
    ...
    def _compute_next_waypoints(self, k=1):
        ...
        # 从已经规划的测试路径的最后一个waypoint开始,继续向前规划K(比如200)个点
        for _ in range(k):
    ...
    def _get_waypoints(self):
        """
        这个函数的主要工作是获取自动驾驶车辆的路径点。具体步骤如下：
            检查路径点队列是否不足，如果不足则添加更多路径点。
            将路径点缓冲区填满。
            将缓冲区中的路径点转换为坐标列表。
            获取当前车辆的路径点和目标路径点。
            清除缓冲区中已经过时的路径点。
            返回路径点列表、目标路径选项、当前路径点和目标路径点。
        """
        ...
        while len(self._waypoint_buffer) < self._buffer_size:
            if self._waypoints_queue:
                ...
                break
        # 从self._waypoint_buffer中获取获取x,y,yaw,将它们存入waypoints列表中
        waypoints = []
        ...
        max_index = -1
        # 遍历self.waypoint_buffer中的每一个点,根据与当前车辆位置的距离关系从buffer中删除过时的点
        for i, (waypoint, _) in enumerate(self._waypoint_buffer):
