...
class CarlaEnv(gym.Env):
    ...
    def _get_scenario_reward(self):
        ...
        k_dev = 0.4
        # 奖励随偏离增大而增大，最大不超过0.6
        r_lane_deviation = (1.0 - np.exp(-k_dev * np.minimum(abs(dis), self.out_lane_thres))) * 0.6
        ...
        if self.scenario_manager.scenario_list[0].reference_actor is not None:
            ...
            if dist < danger_distance_thres:
                ...
                adv_vel = adv_veh.get_velocity()
                ego_vec = np.array([ego_vel.x, ego_vel.y])
                adv_vec = np.array([adv_vel.x, adv_vel.y])
                # 归一化
                norm_ego = np.linalg.norm(ego_vec) + 1e-6
                norm_adv = np.linalg.norm(adv_vec) + 1e-6
                ego_dir = ego_vec / norm_ego
                adv_dir = adv_vec / norm_adv
                # 计算夹角的余弦
                cos_theta = np.dot(ego_dir, adv_dir)
                # 只有相向而行（cos_theta < 0）时才奖励
                if cos_theta < 0:
                    direction_factor = (1.0 - cos_theta) * 0.5  # 相向最大为1，同向为0
                    ratio = (danger_distance_thres - dist) / danger_distance_thres
                    r_proximity = np.clip((ratio ** 2) * direction_factor, 0.0, 1.0)
                else:
                    r_proximity = 0.0

        # 总奖励为三项之和，均为正数
        r = r_collision + r_lane_deviation + r_proximity
        return r
