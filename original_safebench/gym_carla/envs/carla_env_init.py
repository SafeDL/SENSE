...
class CarlaEnv(gym.Env):
    ...
    def _get_scenario_reward(self):
        """
        对于scenario policy策略而言,我们希望给与碰撞奖励
        """
        # 碰撞奖励
        ...
        ego_x, ego_y = get_pos(self.ego_vehicle)
        dis, w = get_lane_dis(self.waypoints, ego_x, ego_y)
        lane_dev_ratio = min(abs(dis) / self.out_lane_thres, 1.0)
        r_lane_deviation = lane_dev_ratio * 0.1

        # 与交通车的距离奖励（continuous proximity reward）
        danger_distance_thres = 10.0  # 危险距离阈值（米）
        r_proximity = 0.0
        ...
        if self.scenario_manager.scenario_list[0].reference_actor is not None:
            ...
            if dist < danger_distance_thres:
                # 线性映射到 [0, 1]，距离越小奖励越高
                r_proximity += (danger_distance_thres - dist) / danger_distance_thres * 0.5
