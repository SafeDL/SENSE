...
class CarlaEnv(gym.Env):
    ...
    def get_static_obs(self, config):
        ...
        # 对于scenario_policy训练而言,速度是随机生成的
        # state = {
        #     'route': np.array(waypoint_xy),
        #     'target_speed': random.uniform(20, 30),
        # }
    ...
    def _get_info(self):
        ...
        info = {
            ...
            'cost': self._get_cost()  # 此为碰撞cost,-1代表存在碰撞
        }
    ...
    def _get_obs(self):
        ...
        lateral_dis, w = get_preview_lane_dis(self.waypoints, ego_x, ego_y)  # w 是路点的切线方向
        yaw = np.array([np.cos(ego_yaw), np.sin(ego_yaw)])  # 构造 ego 车辆当前朝向的二维单位向量
        delta_yaw = np.arcsin(np.cross(w, yaw))  # 计算航向偏差，w 是路点的切线方向
    ...
    #     """ Calculate the step reward. 对于ego车辆训练 """
    #
    #     if self.scenario_manager.running_record[-1]['collision'] == Status.FAILURE:
    #         r_collision = -1
    #     else:
    #         r_collision = 0
    #
    ...
    #     lspeed_lon = np.dot(lspeed, w)  # m/s
    #     r_fast = -1 if lspeed_lon * 3.6 > self.desired_speed else 0
    #
    ...
    #     """
    #     对于scenario01 而言, 奖励设计如下：
    #     Reward = -r_d + r_b
    #     r_d: AV与障碍物的欧氏距离
    #     r_b: 碰撞发生时给予的奖励
    #     """
    #     # 获取AV和障碍物的位置（假设self.ego_vehicle是AV，self.obstacle是障碍物）
    #     ego_loc = self.ego_vehicle.get_location()
    #     reference_actor = self.scenario_manager.scenario_list[0].reference_actor
    #
    #     if reference_actor is not None:
    #         adv_veh = self.scenario_manager.scenario_list[0].reference_actor
    #         r_d = 0.05 * ego_loc.distance(adv_veh.get_location())
    #     else:
    #         r_d = 1.0  # 如果没有障碍物，设置一个较大的距离
    #
    #     # 检查碰撞
    #     if self.scenario_manager.running_record[-1]['collision'] == Status.FAILURE:
    #         r_b = 300.0
    ...
    def _get_scenario_reward(self):
        """
        对于scenario02 而言, REINFORCE奖励设计如下：
        1. 发生碰撞奖励为正（仿真一旦碰撞即终止, r_collision只会在碰撞帧为正）
        2. 偏离车道线奖励为正（偏离越大奖励越大）
        3. 交通车与本车相向而行且近距离时奖励为正
        """
        # 1. 碰撞奖励 (Collision Bonus, r_b)
        is_collision = self.scenario_manager.running_record[-1]['collision'] == Status.FAILURE
        r_collision = 50.0 if is_collision else 0.0

        # 2. 距离奖励 (Distance Metric, -r_d) [cite: 216-217]
        # 论文直接使用欧氏距离。为了数值稳定性，我们可以将其转化为归一化的接近度。
        ego_loc = self.ego_vehicle.get_location()
        adv_veh = self.scenario_manager.scenario_list[0].reference_actor
        ...
        # 将距离转化为 [0, 1] 的奖励，距离越近奖励越大
        # 使用 exponential kernel 比线性更好，避免远距离时的梯度消失
        k_dist = 0.2
        r_distance = np.exp(-k_dist * dist) * 5.0  # 系数可调，作为 dense reward

        # 3. 合理性/约束惩罚 (Penalty, r_p)
        # 论文检查障碍物是否生成得离 route 太近（指初始生成的瞬间）。
        # 如果是在运行过程中的 step reward，我们可以惩罚“不动的障碍物”或者“瞬移”。
        # 这里我们简化为：如果碰撞发生了，但相对速度极低（像是故意挡路而不是撞击），给予惩罚。
        r_penalty = 0.0
        # 示例：如果是生成任务，应该在 reset 时计算 spawn 位置的合理性。
        # 如果是 step 任务，这里可以暂时忽略，或者惩罚 agent 长期静止。

        # 组合奖励
        # 注意：论文中 r_d 是负项，意味着我们要最小化距离。
        # 这里我们用 r_distance (越大越好) 替代了 -r_d。
    ...
    # def _get_scenario_reward(self):
    #     """
    #     对于 SAC 连续动作而言，奖励设计如下：
    #     1. 发生碰撞奖励为正（仿真一旦碰撞即终止, r_collision只会在碰撞帧为正）
    #     2. 偏离车道线奖励为正（偏离越大奖励越大）
    #     3. 交通车与本车近距离时奖励为正
    #     """
    #     # 1. 碰撞奖励（仿真一旦碰撞即终止，r_collision只会在碰撞帧为正）
    #     if self.scenario_manager.running_record[-1]['collision'] == Status.FAILURE:
    #         r_collision = 30.0
    #     else:
    #         r_collision = 0.0
    #
    #     # 2. 车道中心的偏离程度奖励（偏离越大奖励越大，非线性放大）
    #     ego_x, ego_y = get_pos(self.ego_vehicle)
    #     dis, w = get_lane_dis(self.waypoints, ego_x, ego_y)
    #     k_dev = 0.4
    #     # 奖励随偏离增大而增大，最大不超过2.0
    #     r_lane_deviation = (1.0 - np.exp(-k_dev * np.minimum(abs(dis), self.out_lane_thres))) * 2.0
    #
    #     # 3. 与对手车的距离奖励（相向且近距离时奖励为正，距离越近奖励越大，最大1.0）
    #     danger_distance_thres = 20.0
    #     r_proximity = 0.0
    #     ego_loc = self.ego_vehicle.get_location()
    #     if self.scenario_manager.scenario_list[0].reference_actor is not None:
    #         adv_veh = self.scenario_manager.scenario_list[0].reference_actor
    #         dist = ego_loc.distance(adv_veh.get_location())
    #         if dist < danger_distance_thres:
    #             # 线性比例奖励：距离越小，奖励越接近 1
    #             ratio = (danger_distance_thres - dist) / danger_distance_thres
    #             # 也可以用平方或指数增强临近效果
    #             r_proximity = ratio
    #
    #     # 加权组合
    ...
    #     w_proximity = 1.5
    #     r = w_collision * r_collision + w_lane_deviation * r_lane_deviation + w_proximity * r_proximity
    #     return r

    def _get_cost(self):
        # cost for collision
        r_collision = 0
        if (len(self.scenario_manager.running_record) > 0 and
                self.scenario_manager.running_record[-1]['collision'] == Status.FAILURE):
            r_collision = -1
        return r_collision
