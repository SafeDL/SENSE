...
def get_route_scores(record_dict, time_out=60):
    """注意评分代码已经经过修改！！！每个场景单独计算评分，而不是混淆求平均"""
    ...
    min_adv_veh_score = max(0, 1.0 - minium_distance_to_adv_veh / 2.0)  # 距离小于0.5米得满分，距离越大得分越低

    all_scores = {key: round(value/predefined_max_values[key], 2) for key, value in scores.items()}
    all_scores['min_adv_veh_score'] = round(min_adv_veh_score, 2)
    final_score = 0
