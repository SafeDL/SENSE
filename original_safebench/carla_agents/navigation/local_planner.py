...
import carla
from safebench.carla_agents.navigation.controller import VehiclePIDController
from safebench.carla_agents.tools.misc import draw_waypoints, get_speed
...
class LocalPlanner(object):
    ...
    def __init__(self, vehicle, opt_dict={}):
        ...
        self._stop_waypoint_creation = False  # 用于控制是否在划定的测试路径上添加更多的waypoints

        # Base parameters
        self._dt = 1.0 / 20.0
        self._target_speed = 20.0  # Km/h,这里会提供一个初始值,但是会被传入的opt_dict覆盖
        ...
        self._follow_speed_limits = False  # 设置是否要受到本车速度的控制
        ...
        # initializing controller,这里会调用VehiclePIDController类
    ...
    def run_step(self, debug=False):
        ...
        for waypoint, _ in self._waypoints_queue:
            # 在这个循环中,遍历队列中的每一个waypoint来判断是否接近ego_vehicle,凡是接近的全部清除
        ...
        if len(self._waypoints_queue) == 0:  # 当规划的测试路径点队列清空时,一定要停止车辆,因为无路可行
